        ...
        <>
            <div className="cart-items">
                {allProducts?.map((product) => {
                    // Find the cart variant for the current product
                    const cartVariant = cartVariants.find(variant => variant.id === product.id);

                    // If the product is not in the cart, do not render it.
                    if (!cartVariant) {
                        return null;
                    }

                    // Safely find the color object for the selected color
                    const colorObject = product.colors.find(color =>
                        Object.keys(color)[0] === cartVariant.color
                    );

                    // Safely get the image URL from the color object, defaulting to an empty string
